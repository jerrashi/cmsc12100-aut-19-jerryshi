...
###### Task: determine whether a susceptible person gets infects ######

@pytest.mark.parametrize(
    "params",
    read_config_file("gets_infected_at_position_tests.json"))
def test_gets_infected_at_position(params):
    '''
    Test harness for gets_infected_at_position

    Inputs:
      params (dictionary): the test parameters:
        city, position, infection rate
        seed, expected number of calls to the random number generator
        expected result,
    '''

    # use the same seed for all the tests
    if params["seed"] == 20170217:
        # Use pre-generated random checking function
        check_rand = CHECK_RAND_20170217
    else:
        check_rand = gen_check_rand_calls(params["seed"], 2)

    random.seed(params["seed"])

    actual = sir.gets_infected_at_position(params["city"],
                                           params["position"],
    ...
    recreate_msg += "sir.gets_infected_at_position({}, {}, {})"
...
###### Task : Advanced person at position ######
@pytest.mark.parametrize(
    "params",
...
@pytest.mark.parametrize(
    "params",
    read_config_file("calc_avg_num_newly_infected_tests.json"))

def test_calc_avg_num_newly_infected(params):
    '''
    ...
    actual = sir.calc_avg_num_newly_infected(
    ...
    recreate_msg = "To recreate this test run:\n"
    recreate_msg += "  sir.calc_avg_num_newly_infected({}, {}, {}, {}, {}, {})"
    recreate_msg = recreate_msg.format(params["starting_city"],
                                       params["starting_seed"],
                                       params["max_num_days"],
                                       params["infection_rate"],
